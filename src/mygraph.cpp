...
namespace mygraph
{
	...
	typedef struct myNode
	{
		node_id u=-1;
		double cost=0.0;
		myNode(node_id u,double cost):u(u),cost(cost){}
	};
	bool compareByCost(const myNode &a, const myNode &b)
	{
    	return a.cost < b.cost;
	}
	struct CustomCompare {
    bool operator()(const std::pair<node_id, double>& lhs, const std::pair<node_id, double>& rhs) const {
        return lhs.second < rhs.second; // Sắp xếp theo giá trị double (thứ tự không giảm)
    }
};
struct VectorHash {
    size_t operator()(const std::vector<node_id>& vec) const {
        size_t seed = vec.size();
        for (const node_id& num : vec) {
	...
	struct myNodeGreater1
	{
		bool operator()(myNode const &a, myNode const &b) const
		{
			if(a.cost<=b.cost)
				return true;
			else
	...
	double dRand(double a=0.00001, double b=1.0)
	{
		...
		void read_edge_list(string fname)
		{
			...
			while (getline(ifile, sline))
			{
				...
				if (sline[0] != '#')
				{
					...
					if (weighted)
						ss >> wht;
					//logg << "from: " << from << " to: " << to << " weight: " << endL;
					simplifyNode *emptyNode;
					while (to >= this->n)
					{
						emptyNode = new simplifyNode;
						emptyNode->id = this->n;
						emptyNode->wht = dRand(0.00001,1.0);
						adjList.push_back(emptyNode);
						++this->n;
					}
					//logg << "Completed to" << endL;
					while (from >= this->n)
					{
						emptyNode = new simplifyNode;
						emptyNode->id = this->n;
					...
					//logg << "Completed from" << endL;
					add_edge(from, to, wht);
					//logg << "Completed add_edge" << endL;
					++line_number;
				}
			}
			//logg << "Completed reading edge list" << endL;
			ifile.close();
		}
	...
	public:
		...
		tinyEdge(node_id nid, double w = 1.0)
		{
	...
	public:
		...
		void erdos_renyi_undirected(node_id n, double p) // sinh ngẫu nhiên tập cạnh với n đỉnh và tỉ lệ p (ER)
		{
			...
			this->m = 0;
			init_empty_graph();
			uniform_real_distribution<double> dist(0.0001, 1.0);
			...
			for (node_id i = 0; i < n; ++i)
			{
				for (node_id j = i + 1; j < n; ++j)
				{
					...
					{
						++this->m;
						add_edge_immediate(i, j, 1.0);
					}
				}
				adjList[i].wht = dist(gen);
			}
		}
		 ...
		 */
		void add_edge_immediate(unsigned from, unsigned to, double wht = 1)
		{
			tinyEdge FT(to, wht);
		...
		void write_edge_list(string fname)
		{
			...
			for (unsigned i = 0; i < n; ++i)
			{
				...
				for (unsigned j = 0; j < ss; ++j)
				{
					nei_id = adjList[i].neis[j].target;
					if (nei_id > i)
						ifile << i << ' ' << nei_id << ' ' << static_cast<int>(adjList[i].neis[j].weight) << '\n';
				}
			}
			ifile.flush();
			ifile.close();
		 ...
		 */
		double remove_edge(node_id s, node_id t)
		{
